                    ...
                    <div class="modal-body">
                        <form id="composeForm" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea id="composeBody" name="body" class="form-control"></textarea>
                            </div>
                            <div class="mb-3">
                                <label>ضمیمه</label>
                                <input id="composeAttachment" type="file" name="attachments[]" class="form-control" multiple>
                            </div>
                            <button type="button"
                                    id="btnComposeSend"
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <div>
                            ...
                            <div id="viewParticipants" class="d-flex"></div>
                        </div>
                        <button class="btn-close" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body message-scroll" id="viewMessages"></div>
                </div>
            </div>
        </div>

        {{-- FAB --}}
        <button id="fabCompose" class="fab-compose" data-bs-toggle="modal" data-bs-target="#composeModal">
            <span class="mdi mdi-plus"></span>
        </button>
...
@section('script')
    ...
    <script>
        window.CORRESPONDENCE_DATA = {
            authUserId: {{ auth()->id() }},
            users: {!! json_encode($users->mapWithKeys(fn($u) => [$u->id => ['id'=>$u->id,'name'=>$u->name]]) ) !!},
            conversations: {!! json_encode($conversations->map(function($c){
        ...
        $last = $messages->last();
        $mapMsg = fn($m)=>[
            'id'=>'m'.$m->id,'senderId'=>$m->sender_id,'body'=>$m->body,'time'=>$m->created_at,
            'attachments'=>$m->attachments->map(fn($a)=>['id'=>$a->id,'name'=>$a->original_name,'url'=>$a->url])->toArray()
        ];
        return [
            'id'=>$c->id,
            'subject'=>$c->subject,
            ...
            'lastActivity'=>optional($last)->created_at,
            'messages'=>['root'=>$root?$mapMsg($root):null,'replies'=>$replies->map($mapMsg)->values()->toArray()]
        ];
    })) !!}
        };
    </script>
